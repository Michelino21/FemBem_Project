...
Dati = struct( 'name',             test,...
               ...
               'theta',             0.5, ...
               ... % Theta-method ..
               'initialcond',      'sin(2*pi*x).*cos(pi*y)', ...
               ... % Initial condition 
               'exact_sol',        'sin(2*pi*x).*cos(pi*y).*exp(-2*t)',...      
               ... % Definition of exact solution
               'force',            '(5*pi^2 - 2).*sin(2*pi*x).*cos(pi*y)',...  
               ... % Forcing term in space
               'forcetime',        'exp(-2*t)',...
               ... % Forcing term in time        
               'grad_exact_1',     '2*pi.*cos(2*pi*x).*cos(pi*y).*exp(-2*t)',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '-1*pi.*sin(2*pi*x).*sin(pi*y).*exp(-2*t)',...    
